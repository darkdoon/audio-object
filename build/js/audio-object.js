...
(function (window) {
	...
	function automateToValue(param, value1, value2, time1, time2, curve) {
		...
		// 'linear'.
		curve = !time2 ? 'step' : time2 === time1 ? 'step' : curve ? curve : 'linear';
	...
	function getValue(cues, time) {
		...
		return time > time2 ? value2 : getters[curve](value1, value2, time1, time2, time);
	}
	...
	function defineAudioProperty(object, name, audio, data) {
		...
		if (param ? !isAudioParam(param) : !data.set) {
			throw new Error('AudioObject.defineAudioProperty requires EITHER data.param to be an AudioParam' + 'OR data.set to be defined as a function.');
		}
	...
	function AudioObject(audio, input, output, params) {
		...
		if (input) {
			inputs.set(this, isAudioNode(input) ? { 'default': input } : assign({}, input));
		}

		// Keep a map of outputs in AudioObject.outputs
		if (output) {
			outputs.set(this, isAudioNode(output) ? { 'default': output } : assign({}, output));
		}
	...
	var prototype = {
		trigger: function trigger(time, type) {
			var args = arguments;
		...
		automate: function automate(name, value, time, curve, duration) {
